...
theta = 0;      % orientation angle, degree measure, south=0, west=90, east=-90
gamma_angle = 36;     % inclination angle, degree measure, horizontally mounted: gamma_angle=0
Norm = 614;     % normalization of energy values, 614 W/m^2 because this is highest value for gamma_angle=36 and theta=0 at noon in Jun in Greenwich
...
off_set = 0.9;     %off_set (load_profile): C(t)=off_set (Constant Load Profile),C(t)=scale*C_bis+off_set (Business Load Profile), C(t)=scale*C_res+off_set (Residential Load Profile) 
scale = 1;         %scale (load_profile): C(t)=off_set (Constant Load Profile),C(t)=scale*C_bis+off_set (Business Load Profile), C(t)=scale*C_res+off_set (Residential Load Profile) 
time_step_when_data_starts = 17;  %to read in the data from text file (17 for June in Greenwich)
time_step_when_data_ends = T - time_step_when_data_starts + 2; %+2 for Greenwich
...
%the following for-loop calculates the the energy generation profile of 1 PV cell out of the N PV cells
...
for t = 1:T
   ...
   at_bt_ct(t,1) = sind(delta)*sind(lat)*cosd(gamma_angle)+cosd(delta)*cosd(lat)*cosd(gamma_angle)*cosd(omega);
   at_bt_ct(t,2) = cosd(delta)*sind(gamma_angle)*sind(omega);
   at_bt_ct(t,3) = -sind(delta)*cosd(lat)*sind(gamma_angle)+cosd(delta)*sind(lat)*sind(gamma_angle)*cosd(omega);
   ...
   Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta(t,4) = I_diffuse(theta,gamma_angle,t,Norm,N,Input_t_GHI_DHI_DNI_Load, Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta,at_bt_ct);

   % Calculation of the ground-reflected irradiance I_g = GHI*albedo*(1-cosd(gamma_angle))/2
   Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta(t,5) = I_ground(gamma_angle,albedo,t,Norm,N,Input_t_GHI_DHI_DNI_Load);
...
if (N==1)
   [f,minimum,maximum,optimum_angles] = print_1D_graph(gamma_angle,N,T,Norm,Input_t_GHI_DHI_DNI_Load,Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta,at_bt_ct,range_of_optimum_points);
else if (N==2)
    [f,minimum,maximum,optimum_angles] = print_2D_graph(gamma_angle,N,T,Norm,Input_t_GHI_DHI_DNI_Load,Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta,at_bt_ct,range_of_optimum_points);
    else if (N==3)
    	[f,minimum,maximum,optimum_angles] = print_3D_graph(gamma_angle,N,T,Norm,Input_t_GHI_DHI_DNI_Load,Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta,at_bt_ct,range_of_optimum_points);
        else 
