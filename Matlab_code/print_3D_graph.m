function [f,minimum,maximum,optimum_angles] = print_3D_graph(gamma_angle,N,T,Norm,Input_t_GHI_DHI_DNI_Load,Output_t_I_Ib_ReindlId_Ig_cosAOI_omega,at_bt_ct,range_of_optimum_points)
    ...
    for theta_1 = -90:1:90
    	for theta_2 = -90:1:90 
        	for theta_3 = -90:1:90 
                ...
                theta(2) = theta_2;
                theta(3) = theta_3;
                V(theta_1+91,theta_2+91,theta_3+91) = normalized_energy_drawn_from_the_main_grid(gamma_angle,N,T,Norm,Input_t_GHI_DHI_DNI_Load,Output_t_I_Ib_ReindlId_Ig_cosAOI_omega,theta,at_bt_ct);
            end
        end
    end
    ...
    for i = 1:2
        ...
        yticks(-90:45:90)
        zticks(-90:45:90)
        % set(gca,'Ydir','reverse') % This should be active for the second column in Table V (see reference in thesis)
