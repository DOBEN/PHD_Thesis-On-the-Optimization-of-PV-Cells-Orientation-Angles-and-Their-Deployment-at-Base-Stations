function [ I_d ] =  I_diffuse(theta,gamma_angle,t,Norm,N,Input_t_GHI_DHI_DNI_Load, Output_t_I_Ib_ReindlId_Ig_At_omega_cosZeta,at_bt_ct)
...
I_d = (DHI/(Norm*N))*((A_t*max(0,cos_of_AOI)/max(0.000001,cos_of_zeta))+(1-A_t)*((1+cosd(gamma_angle))/2)*(1+sqrt(max(0,DNI*cos_of_zeta)/GHI)*(sind(gamma_angle/2)^3)));
end
